    ...
    <nav className="bg-white p-4 shadow-md transition-shadow duration-300 hover:shadow-lg">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className="hidden md:flex space-x-8 justify-center w-full">
          ...
          <Link href="/entradas" className={styles.navLink}>
            ENTRADAS
          </Link>
          ...
          <div className="text-black text-2xl font-bold">
            <Link href="/" className={styles.navLogo}>
              <Box className={styles.logo}>
                <Image src="/images/logoTrucho.svg" alt="Logo de Astroboy XL" width={30} height={30} className={styles.image} />
              </Box>
            </Link>
          </div>
          <button
            onClick={toggleMenu}
            className="text-black focus:outline-none transition-transform duration-300 transform hover:scale-110"
          >
            <svg
              className="w-6 h-6"
              fill="none"
      ...
      {isOpen && (
        <div className="md:hidden mt-2 space-y-2 text-center bg-white shadow-lg transition-transform duration-300 transform translate-y-0 ease-in-out">
          <Link href="/" className={`${styles.navLink} block`}>
            EVENTO
          </Link>
          <Link href="/entradas" className={`${styles.navLink} block`}>
            ENTRADAS
          </Link>
          <Link href="/merch" className={`${styles.navLink} block`}>
            MERCH
          </Link>
          <Link href="/social" className={`${styles.navLink} block`}>
            CONTRIBUIDORES
          </Link>
        </div>
      )}
